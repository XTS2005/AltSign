...
public enum LdidBridge {
    ...
    public static func entitlements(at url: URL) throws -> String {
        ...
        if isDir.boolValue {
            guard let execURL = findExecutable(at: url) else {
                throw Error.operationFailed("在包中定位可执行文件失败：\(url.path)")
            }
            targetURL = execURL
        } else {
            targetURL = url
        ...
        do {
            ...
            throw Error.operationFailed("解析 Mach-O 权限失败：\(error.localizedDescription)")
        }
    }
    ...
    private static func buildCertificateChain(p12Data: Data) throws -> Data {
        ...
        guard let key, let cert else {
            throw Error.operationFailed("密钥或证书缺失")
        }
        
        guard let certificates = OPENSSL_sk_new_null() else {
            throw Error.operationFailed("分配失败")
        }
        ...
        guard let rootCert = readCertFromPEM(AppleRootCertificateData) else {
            ...
            throw Error.operationFailed("在证书链打包期间解析 Apple Root CA 证书失败\n原因：\(openSSLErr)")
        }
        ...
        guard let wwdrCert = readCertFromPEM(wwdrData) else {
            let openSSLErr = getOpenSSLError()
            throw Error.operationFailed("在证书链打包期间解析 Apple WWDR 证书失败\n原因：\(openSSLErr)")
        }
        ...
        guard let outputP12 = PKCS12_create("", "", key, cert, certificates, 0, 0, 0, 0, 0) else {
            let openSSLErr = getOpenSSLError()
            throw Error.operationFailed("在证书链打包期间打包证书链失败\n原因：\(openSSLErr)")
        }
        ...
        guard let outputData = CertificatesManager.dataFromBIO(outputBio) else {
            let openSSLErr = getOpenSSLError()
            throw Error.operationFailed("在证书链打包期间获取打包的链数据失败\n原因：\(openSSLErr)")
        }
    ...
    public static func sign(
        ...
        if status != 0 {
            let message = errorPtr.map { String(cString: $0) } ?? "ldid sign 失败"
